...
def export_excel():
    df = pd.read_csv(DATA_FILE)
    df.to_excel(os.path.join(OUTPUT_DIR, "expense_report.xlsx"), index=False)
    print("\n✔ Exported_to_expense_report.xlsx\n")
...
def generate_chart():
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "expense_chart.png"))
    plt.close()
